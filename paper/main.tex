...
This paper introduces \textit{NanoBubbleEval}, the first structured resource for nanobubble and nanocarrier literature, together with the evaluation infrastructure needed to validate automated extraction into it. The resource is organised around an 18-field schema co-designed with the structure of the field, instantiated on a 51{,}566-abstract deduplicated warehouse harvested across six public scholarly APIs and distilled via the released pipeline into an 8{,}006-record high-precision domain core (the core CSV itself is regenerable from the included scripts and is scheduled for v1.1). The benchmark in this paper plays a specific role in this larger effort: it is the validation harness that demonstrates the resource is usable today for automated extraction, and it surfaces the three failure modes the metric suite is designed to detect. Schema-fill hallucination, unit drift, and cite-hallucination are not framed as the primary contribution; they are what makes automated population of the resource hard, and they motivate the three metrics that govern its scoring protocol.
...
A 51{,}566-record deduplicated warehouse (released in v1.0), a methodology and pipeline for distilling an 8{,}006-record high-precision domain core (scheduled for v1.1), an 18-field schema co-designed with the structure of the field, and a complete harvest-to-evaluation pipeline that takes raw scholarly-API queries to a scoring-ready release.
