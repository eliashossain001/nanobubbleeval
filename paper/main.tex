...
Worked examples per category are scheduled for v1.1 alongside the
500-record gold-pool release; v1.0 reports the taxonomy itself
together with per-baseline aggregate evidence in
...
\begin{table}[h]
\centering
\caption{Worked B3 examples on the gold-hard tier covering five error
categories. \texttt{record\_id} is abbreviated to its DOI tail.
Reproduced verbatim from the released gold-hard and B3 prediction files.}
\label{tab:error_examples}
\small
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.20}
\begin{tabularx}{\linewidth}{@{}p{0.16\linewidth} p{0.13\linewidth} p{0.10\linewidth} p{0.10\linewidth} X p{0.13\linewidth}@{}}
\toprule
\textbf{record\_id (tail)} & \textbf{Field} & \textbf{Gold} & \textbf{B3 pred} & \textbf{Predicted span / explanation} & \textbf{Error type} \\
\midrule
10.1155/2017/8234712 & \texttt{stability} & \NR{} & 96~h & ``slowly released up to 96 h'' --- B3 emits a release-time number when stability is not reported. & Schema-fill hallucination \\
10.1021/acsami.5b07778 & \texttt{payload} & gas & gas (correct) & B3's evidence span ``formation mechanism of GU-Liposome were investigated'' does not contain ``gas''. & Cite-hallucination \\
10.1007/s12032-025-03200-7 & \texttt{zeta\_potential} & \NR{} & \NR{} & B3 correctly abstains when the abstract does not state a zeta potential. & Correct abstention \\
10.2166/wqrj.2025.052 & \texttt{payload} & ``hydrogen peroxide; ozone'' & \NR{} & Review abstract: ``hydrogen peroxide and ozone nanobubbles''. B3 misses the payload list. & Review-abstract miss \\
10.1080/03639045.2025.2562181 & \texttt{size} & 332.1~nm & 332.1~nm & B3 evidence quotes ``particle size 332.1 nm'' --- value, unit, and span all consistent. & Numeric+unit-correct \\
\bottomrule
\end{tabularx}
\end{table}
...
nanobubble-vs-nanoparticle slice gap. Worked examples per category
are scheduled for v1.1 alongside the gold-pool release.
