...
% ============================================================================
\begin{abstract}
Nanobubble and nanocarrier research spans biomedical delivery, imaging, environmental remediation, agriculture, and colloidal physics, yet its quantitative findings remain scattered across paper text, tables, and figures. We introduce \textbf{NanoBubbleEval}\footnote{Dataset: \url{https://huggingface.co/datasets/EliasHossain/nanobubbleeval}. Code: \url{https://github.com/eliashossain001/nanobubbleeval}.}, the first structured dataset and evaluation framework for extracting nanobubble and nanocarrier properties from literature. Built from a 51{,}566-abstract deduplicated warehouse together with a methodology for distilling an 8{,}006-record domain core and a 500-record gold pool, the v1.0 release ships the deduplicated warehouse and a 40-record gold-hard seed evaluation tier annotated under an 18-field schema; the high-precision core, the full gold pool, and the gold-lite extension tier are scheduled for v1.1. It supports three task views: schema extraction, unit-normalised numerical grounding, and verbatim evidence attribution. We benchmark rule-based extraction, BioBERT-SQuAD-v2, and Qwen2.5-7B-Instruct using three domain-aligned metrics: abstention-calibrated F1, tolerance-bounded numerical match, and answer--evidence consistency. Qwen2.5-7B-Instruct achieves the best macro abstention-calibrated F1 (0.758), outperforming BioBERT-SQuAD-v2 (0.545) and rule-based extraction (0.493), but shows reduced robustness on review abstracts and lower evidence consistency than span-based extractors. NanoBubbleEval provides a deterministic harvest-to-evaluation pipeline, slice annotations, leakage auditing, Croissant 1.1 metadata, and an open-source Python evaluation harness, establishing foundational infrastructure for machine-readable nanobubble and nanocarrier literature.
\end{abstract}
