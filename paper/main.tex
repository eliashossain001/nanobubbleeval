...
\textit{NanoBubbleEval} is intended to support more reliable scientific information extraction by exposing three failure modes (schema-fill hallucination, unit drift, and cite-hallucination) that current single-task benchmarks evaluate in isolation. By treating \NR{} as a first-class output and by requiring verbatim evidence spans, the benchmark discourages downstream pipelines from acting on unsupported emissions in tasks such as literature review, database construction, and meta-analysis. We anticipate two principal risks. First, headline numbers reported on the 40-record gold-hard tier should not be extrapolated to other materials-science or biomedical extraction domains without re-validation; pipelines that consume extracted property values should preserve the abstention and evidence-provenance signals that the metric suite measures. Second, the warehouse is dominated by English-language abstracts indexed by a small set of scholarly APIs, which biases the benchmark toward the existing distribution of English biomedical and environmental publications. We mitigate these risks by releasing the schema, the unit-normalisation tables, and the full evaluation harness as a Python package that downstream users can adapt to new domains, by documenting the tier-curation and slice-tagging procedures end-to-end, and by versioning the release so that v1.1 can extend the gold-hard tier with multi-rater adjudication and full-text grounding without invalidating v1.0 results.
