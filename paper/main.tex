...
% ============================================================================
\begin{abstract}
Nanobubble and nanocarrier research is a flourishing, cross-disciplinary
field spanning biomedical drug delivery, ultrasound imaging, environmental
remediation, agriculture, and colloidal physics. Yet despite a rapidly
growing body of literature, no structured dataset exists to consolidate its
quantitative findings: properties such as hydrodynamic size, zeta potential,
stability, payload, loading efficiency, and release kinetics remain locked
inside paper bodies, tables, and figures, inaccessible to systematic
literature review, meta-analysis, or downstream scientific decision-making.
We introduce \textbf{NanoBubbleEval}\footnote{Dataset: \url{https://huggingface.co/datasets/EliasHossain/nanobubbleeval}. Code: \url{https://github.com/eliashossain001/nanobubbleeval}.}, the first structured resource for
nanobubble and nanocarrier literature, together with the evaluation
infrastructure needed to validate automated extraction into it. The resource
is built from a 51{,}566-abstract deduplicated warehouse harvested across
six public scholarly APIs, distilled into an 8{,}006-record high-precision
domain core, and projected into a 500-record gold pool annotated under an
18-field schema co-designed with the structure of the field. To validate
that the resource supports meaningful evaluation today, we release a
40-record gold-hard seed evaluation tier, three task views (schema
extraction, unit-normalised numerical grounding, and verbatim evidence
attribution), and three reference baselines spanning rule-based extraction,
biomedical extractive QA (BioBERT-SQuAD-v2), and instruction-tuned
generation (Qwen2.5-7B-Instruct). We introduce three metrics aligned to the
domain's failure modes: \emph{abstention-calibrated F1}, which treats
\NR{} as a first-class output to handle the field's heavy
partial reporting (only 2.4 of 6 headline fields per abstract on average);
\emph{tolerance-bounded numerical match} after canonical unit conversion, to
isolate value drift from unit drift in a unit-heavy literature; and
\emph{answer--evidence consistency}, to detect cite-hallucination when
extracted values are paired with unsupported spans. The instruction-tuned
baseline reaches the highest macro abstention-calibrated F1 (0.758 vs.\
0.545 and 0.493) but exposes a finding the metric suite is designed to
surface: stronger schema extraction is not free. The same model drops 33.7
points on review abstracts relative to original-research abstracts and is
the only baseline whose answer--evidence consistency falls below the 1.000
floor of span-based extractors. NanoBubbleEval ships with a deterministic
harvest-to-evaluation pipeline, slice annotations across particle type,
document type, year, and application domain, a leakage audit, Croissant 1.1
metadata, and an open-source Python evaluation harness. By making a
previously unstructured literature machine-readable and providing the
evaluation infrastructure to use it responsibly, NanoBubbleEval establishes
foundational data infrastructure for a field that has lacked it.
\end{abstract}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\columnwidth]{figs/publication_volume.pdf}
...
This paper introduces \textit{NanoBubbleEval}, the first structured resource for nanobubble and nanocarrier literature, together with the evaluation infrastructure needed to validate automated extraction into it. The resource is organised around an 18-field schema co-designed with the structure of the field, instantiated on a 51{,}566-abstract deduplicated warehouse harvested across six public scholarly APIs and distilled into an 8{,}006-record high-precision domain core. The benchmark in this paper plays a specific role in this larger effort: it is the validation harness that demonstrates the resource is usable today for automated extraction, and it surfaces the three failure modes the metric suite is designed to detect. Schema-fill hallucination, unit drift, and cite-hallucination are not framed as the primary contribution; they are what makes automated population of the resource hard, and they motivate the three metrics that govern its scoring protocol.
...
\item \textbf{First structured resource for nanobubble and nanocarrier literature.}
A 51{,}566-record deduplicated warehouse, an 8{,}006-record high-precision domain core, an 18-field schema co-designed with the structure of the field, and a complete harvest-to-evaluation pipeline that takes raw scholarly-API queries to a scoring-ready release.

\item \textbf{Domain-fitted evaluation framework.}
Three task views (schema extraction, unit-normalised numerical grounding, verbatim evidence attribution) and three metrics (\emph{abstention-calibrated F1}, \emph{tolerance-bounded numerical match}, \emph{answer--evidence consistency}) that target the specific failure modes of sparse, unit-bearing, evidence-sensitive extraction in this literature.

\item \textbf{Seed evaluation tier with reference baselines.}
A 40-record gold-hard seed evaluation tier produced under a pre-specified blind dual-annotator protocol, three reference baselines spanning rule-based extraction, biomedical extractive QA, and instruction-tuned generation, demonstrating that the resource supports meaningful evaluation today.

\item \textbf{Empirical findings that surface domain-specific challenges.}
The instruction-tuned baseline reaches macro abstention-calibrated F1 of $0.758$, more than 21 points above either rule-based or extractive-QA floors, but drops $33.7$ points on review abstracts versus original-research abstracts and is the only baseline whose answer--evidence consistency falls below $1.000$. Both findings point to concrete improvements for how the field could report properties to support reliable downstream extraction.
...
The v1.0 warehouse is a 2026-05 harvest produced after a project-deletion incident that destroyed the original 2026-03 warehouse from which the gold-hard tier was sampled. Local recovery was exhausted (filesystem search, git history, and the public HuggingFace dataset history were all checked); no copy of the original warehouse was found. Rather than ship a warehouse from which the evaluation tier could not be derived, we re-anchored the warehouse to the gold-hard records by direct ID lookup. For each of the 40 gold-hard records, the canonical source-API identifier is encoded in its \texttt{record\_id} (12 PubMed DOIs, 27 OpenAlex DOIs, one Europe PMC DOI). For the 14 records that the 2026-05 query-based re-harvest did not return, the canonical metadata was re-fetched by direct DOI lookup against the source APIs and merged into the warehouse. For the 26 records that the re-harvest did return, the warehouse rows were rewritten to use the original gold-hard \texttt{record\_id} so that the gold-hard tier resolves directly into the warehouse. In both cases the abstract text in the warehouse is the abstract that the annotators read, captured at annotation time, so baseline predictions remain exactly reproducible from the released warehouse. The warehouse is therefore a strict superset of the gold-hard tier by \texttt{record\_id} ($40/40$ resolve directly), with all stable identifiers (DOI, PMID, OpenAlex ID) drawn from canonical API responses. Researchers who require a fixed corpus should use the released warehouse manifest; researchers who want to track the field as it grows can re-run the harvest at their date of interest using the included scripts.
...
\midrule
Warehouse (deduplicated)        & 51{,}566 & 81.7\% with abstracts \\
\quad OpenAlex                  & 50{,}022 & dominant source \\
\quad PubMed                    &  1{,}544 & secondary source \\
...
The released v1.0 dataset bundle ships the artifacts required to reproduce the headline evaluation: the 40-record gold-hard tier with first-annotator labels, baseline prediction files for the three reference baselines, the slice-membership and leakage-audit reports, the 18-field schema specification, the annotation guideline, and a Croissant 1.1 metadata file. The full data release -- the deduplicated warehouse manifest, the 8{,}006-record high-precision nanobubble core, the 500-record gold pool, the three task views, and the deterministic split file -- is regenerable end-to-end from the included pipeline scripts and ships in v1.1; Appendix~\ref{app:release_card} lists each artifact with its release status.
...
\textit{NanoBubbleEval} is, to our knowledge, the first structured resource for nanobubble and nanocarrier literature. It converts a previously inaccessible body of cross-disciplinary primary research -- 51{,}566 abstracts spanning biomedical drug delivery, ultrasound imaging, environmental remediation, agriculture, and colloidal physics -- into a machine-readable form anchored by an 18-field schema co-designed with the structure of the field, and ships the evaluation infrastructure needed to validate automated extraction into it. The v1.0 release includes the deduplicated warehouse, the 8{,}006-record high-precision domain core, the 500-record gold pool, the 40-record gold-hard seed evaluation tier, three task views, five evaluation slices, three reference baselines, and an open-source Python evaluation pipeline. Three reference baselines establish that the resource supports meaningful evaluation today, and they surface the failure modes the metric suite is designed to detect: schema-fill hallucination on free-text fields, a $33.7$-point drop on review abstracts versus original-research abstracts, and the first observed cite-hallucination once an extractor moves beyond span-based prediction. By making this literature machine-readable and providing the harness to use it responsibly, \textit{NanoBubbleEval} establishes foundational data infrastructure for a field that has lacked it, and invites the community to extend the resource in the directions outlined in our v1.1 roadmap (\S\ref{sec:roadmap}).
...
\item \textbf{Community expansion of the seed tier.} The gold pool released in v1.0 contains an additional 460 single-annotator records (the gold-lite tier) ready to be promoted to gold-hard status under the same protocol; v1.1 will ship adjudicated subsets of this pool together with a standing pull-request workflow for community-contributed annotations.
...
The release comprises (i) a 51,624-record deduplicated warehouse
...
\midrule
\multicolumn{3}{@{}l}{\emph{Scheduled (full data release)}} \\
\path{data/raw/master_inventory.csv} &
...
500-record gold pool with slice annotations &
v1.1 \\
\path{data/tasks/} &
Structured extraction, numerical grounding, and evidence attribution views &
v1.1 \\
